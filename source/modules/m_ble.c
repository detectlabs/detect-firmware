...
static void advertising_init(void)
{
    ...
    err_code = ble_advertising_init(p_m_advertising, &init);
    APP_ERROR_CHECK(err_code);

    ble_advertising_conn_cfg_tag_set(p_m_advertising, APP_BLE_CONN_CFG_TAG);
}


/**@brief Function for starting advertising.
 */
void m_ble_advertising_start(bool erase_bonds)
{
    if (erase_bonds == true)
    {
        delete_bonds();
    ...
    {
        uint32_t err_code = ble_advertising_start(p_m_advertising, BLE_ADV_MODE_FAST);
        APP_ERROR_CHECK(err_code);
...
{
    return ble_advertising_restart_without_whitelist(p_m_advertising);
}

uint32_t m_sd_ble_gap_disconnect(void)
{
    return sd_ble_gap_disconnect(*p_m_conn_handle, BLE_HCI_REMOTE_USER_TERMINATED_CONNECTION);
}
...
static void on_conn_params_evt(ble_conn_params_evt_t * p_evt)
{
    ...
    {
        err_code = sd_ble_gap_disconnect(*p_m_conn_handle, BLE_HCI_CONN_INTERVAL_UNACCEPTABLE);
        APP_ERROR_CHECK(err_code);
    }
}
...
static void ble_evt_handler(ble_evt_t const * p_ble_evt, void * p_context)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        ...
        case BLE_GAP_EVT_CONNECTED:
            ...
            APP_ERROR_CHECK(err_code);
            *p_m_conn_handle = p_ble_evt->evt.gap_evt.conn_handle;
            err_code = nrf_ble_qwr_conn_handle_assign(&m_qwr, *p_m_conn_handle);
            APP_ERROR_CHECK(err_code);
            break;
