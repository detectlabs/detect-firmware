...
static struct
{
    drv_vl53l0x_twi_cfg_t const * p_cfg;
} m_vl53l0x;

ret_code_t i2c_write(uint8_t deviceAddr, uint8_t * pdata, size_t size, bool stop);
ret_code_t _i2c_read(uint8_t devAddr, uint8_t regAddr, uint8_t * pdata, size_t size);
...
// i2c_stop();
    ...
    data_temp[0] = reg;
    data_temp[1] = value;
    i2c_write(address, data_temp, 2, true);
}
...
// i2c_stop();
    ...
    data_temp[0] = reg;
    data_temp[1] = ((value >> 8) & 0xFF);
    data_temp[2] = value & 0xFF;
    i2c_write(address, data_temp, 3, true);
...
// i2c_stop();
    ...
    data_temp[0] = reg;
    data_temp[1] = ((value >> 24) & 0xFF);
    data_temp[2] = ((value >> 16) & 0xFF);
    data_temp[3] = ((value >>  8) & 0xFF);
    data_temp[4] = value & 0xFF;
    i2c_write(address, data_temp, 5, true);
...
//!  last_status = Wire.endTransmission();
//!
//!  Wire.requestFrom(address, (int8_t)1);
//!  value = Wire.read();
//!
...
//!  last_status = Wire.endTransmission();
//!
//!  Wire.requestFrom(address, (int8_t)2);
//!  value  = (int16_t)Wire.read() << 8; // value high byte
...
//   i2c_start();
//   i2c_write(address | 1);
//   value = (int16_t)_i2c_read() << 8;
//   value |= _i2c_read(0);

  _i2c_read(address, reg, data_temp, 2);
  value  = (data_temp[0] << 8) | (data_temp[1] & 0xff);
...
//!  last_status = Wire.endTransmission();
//!
//!  Wire.requestFrom(address, (int8_t)4);
//!  value  = (int32_t)Wire.read() << 24; // value highest byte
...
//   i2c_start();
//   i2c_write(address | 1);
//   value =  (int32_t)_i2c_read() << 24;
//   value |= (int32_t)_i2c_read() << 16;
...
//    i2c_stop();
    ...
    i2c_write(address, data_temp, 7, true);
...
//    i2c_stop();
   ...
   _i2c_read(address, reg, dst, count);
...
bool setMeasurementTimingBudget(int32_t budget_us)
{
  ...
  int16_t const StartOverhead      = 1320; // note that this is different than the value in get_
  int16_t const EndOverhead        = 960;
  int16_t const MsrcOverhead       = 660;
  int16_t const TccOverhead        = 590;
  int16_t const DssOverhead        = 690;
  int16_t const PreRangeOverhead   = 660;
  int16_t const FinalRangeOverhead = 550;

  int32_t const MinTimingBudget = 20000;

  if (budget_us < MinTimingBudget) { return false; }

  int32_t used_budget_us = StartOverhead + EndOverhead;
  ...
  if (enables.final_range)
  {
    ...
    int32_t final_range_timeout_us = budget_us - used_budget_us;
    ...
    int16_t final_range_timeout_mclks =
...
int32_t getMeasurementTimingBudget(void)
{
  ...
  int16_t const StartOverhead     = 1910; // note that this is different than the value in set_
  int16_t const EndOverhead        = 960;
  int16_t const MsrcOverhead       = 660;
  int16_t const TccOverhead        = 590;
  int16_t const DssOverhead        = 690;
  ...
  int32_t budget_us = StartOverhead + EndOverhead;
...
{
  int8_t vcsel_period_reg = encodeVcselPeriod(period_pclks);
  ...
  if (type == VcselPeriodPreRange)
  {
    ...
    int16_t new_pre_range_timeout_mclks =
    ...
    int16_t new_msrc_timeout_mclks =
  ...
  else if (type == VcselPeriodFinalRange)
  {
    ...
    int16_t new_final_range_timeout_mclks =
  ...
  int8_t sequence_config = readReg(SYSTEM_SEQUENCE_CONFIG);
  writeReg(SYSTEM_SEQUENCE_CONFIG, 0x02);
...
void startContinuous(int32_t period_ms)
{
  ...
  if (period_ms != 0)
  {
    ...
    int16_t osc_calibrate_val = readReg16Bit(OSC_CALIBRATE_VAL);
...
//   }
  ...
  int16_t range = readReg16Bit(RESULT_RANGE_STATUS + 10);
...
// Note: the original function returned a int32_t, but the return value is
// always stored in a int16_t.
int16_t decodeTimeout(int16_t reg_val)
{
  // format: "(LSByte * 2^MSByte) + 1"
  return (int16_t)((reg_val & 0x00FF) <<
         (int16_t)((reg_val & 0xFF00) >> 8)) + 1;
}

// Encode sequence step timeout register value from timeout in MCLKs
// based on VL53L0X_encode_timeout()
// Note: the original function took a int16_t, but the argument passed to it
// is always a int16_t.
...
static uint32_t reg_read(uint8_t reg_addr, uint8_t * p_reg_val)
{
    ...
    err_code = nrf_drv_twi_rx( m_vl53l0x.p_cfg->p_twi_instance,
                               m_vl53l0x.p_cfg->twi_addr,
                               p_reg_val,
                               1 );
    RETURN_IF_ERROR(err_code);
...
static __inline uint32_t twi_open(void)
{
    ...
    err_code = twi_manager_request(m_vl53l0x.p_cfg->p_twi_instance,
                                   m_vl53l0x.p_cfg->p_twi_cfg,
                                   NULL,
                                   NULL);
//   RETURN_IF_ERROR(err_code);
...
{
    nrf_drv_twi_disable(m_vl53l0x.p_cfg->p_twi_instance);

    nrf_drv_twi_uninit(m_vl53l0x.p_cfg->p_twi_instance);
...
ret_code_t i2c_write(uint8_t deviceAddr, uint8_t * pdata, size_t size, bool stop)
{
    ...
    do
    {
       ret = nrf_drv_twi_tx(m_vl53l0x.p_cfg->p_twi_instance, m_vl53l0x.p_cfg->twi_addr, pdata, size, stop);
       if (NRF_SUCCESS != ret)
       {
           break;
       }
...
ret_code_t _i2c_read(uint8_t devAddr, uint8_t regAddr, uint8_t * pdata, size_t size)
{
    ...
    do
    {
       ret = nrf_drv_twi_tx(m_vl53l0x.p_cfg->p_twi_instance, m_vl53l0x.p_cfg->twi_addr, &regAddr, 1, true);
       if (NRF_SUCCESS != ret)
       {
           break;
       }
       ret = nrf_drv_twi_rx(m_vl53l0x.p_cfg->p_twi_instance, m_vl53l0x.p_cfg->twi_addr, pdata, size);
    }while (0);
    return ret;
}
